module Revival
	...
	def revive_user(user)
		...
		if eliminated
			# If there is a contract order, preserve it.
			if user.team.participation.game.remaining_teams.count > 5
				...
				contract.save
			elsif user.team.participation.game.remaining_teams.count != 5
				contracts = Contract.where(target_id: user.team.id, completed: true)
				contracts.each do |c|
					unless c.participation.team.eliminated?
						c.end = nil
						c.completed = false
